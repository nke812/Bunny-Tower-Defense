...
func DMGED(quantidade):
    ...
    if vida <= 0:
        ...
        $"scaling node/Unholy Phantasm".modulate = Color(0.957, 0.478, 0.965, 0.0)
...
func aplicar_knockback(distancia: float) -> void:
    ...
    if pai is PathFollow2D:
        ...
        tween_A.tween_property($"scaling node/Unholy Phantasm", "modulate", Color(2.0, 2.0, 0.289, 1.0), 0.3)
        tween_A.tween_property($"scaling node/Unholy Phantasm", "modulate", Color(1, 1, 1, 1), 0.4)
        speed = speed_base / 10
