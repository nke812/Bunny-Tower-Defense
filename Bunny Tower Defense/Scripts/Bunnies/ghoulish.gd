...
@onready var Ghoulish = $Pega/Path2D/PathFollow2D/Ghoulish
...
func auraMAISego():
    Ghoulish.modulate = Color(1, 1, 1)
    $AURA.play("default")
    
    var tween = create_tween()

    tween.tween_property(Ghoulish, "modulate", Color(2, 2, 2, 1), 0.3)
 
    tween.tween_property(Ghoulish, "modulate", Color(1, 1, 1, 1), 0.4)
