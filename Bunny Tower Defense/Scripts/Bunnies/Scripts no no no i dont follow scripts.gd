...
func atacar(alvo):
    if alvo.has_method("DMGED"):
        ...
        $"Node2D/rotate those pretty litle hands for me/RookieHands_Attack".play()
...
func _on_button_button_down() -> void:
    ...
    if hud:
        ...
        hud.get_node("HUD_Shop/HudBgDown/BunnySel").texture = load("res://Assets/Others/Others/searching for something new are you/C void head.PNG")
...
func auraMAISego():
    ...
    $"Node2D/rotate those pretty litle hands for me/RookieHands_Attack".modulate = Color(1, 1, 1)
    $AURA.play("default")
    
    var tween = create_tween()

    tween.tween_property($Node2D/Rookie, "modulate", Color(2, 2, 2, 1), 0.3)
    tween.parallel().tween_property($"Node2D/rotate those pretty litle hands for me/RookieHands_Attack", "modulate", Color(2, 2, 2, 1), 0.3)
 
    tween.tween_property($Node2D/Rookie, "modulate", Color(1, 1, 1, 1), 0.4)
    tween.parallel().tween_property($"Node2D/rotate those pretty litle hands for me/RookieHands_Attack", "modulate", Color(1, 1, 1, 1), 0.4)
